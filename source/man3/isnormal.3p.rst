...
::
   ...
   ISNORMAL(3P)            POSIX Programmer's Manual           ISNORMAL(3P)
...
::
          ...
          is normal (neither zero, subnormal, infinite, nor NaN). First, an
          argument represented in a format wider than its semantic type is
          converted to its semantic type. Then determination is based on
...
::
          ...
          The isnormal() macro shall return a non-zero value if and only if
...
::
          ...
          fpclassify(3p), isfinite(3p), isinf(3p), isnan(3p), signbit(3p)
...
::
   ...
   IEEE/The Open Group               2017                      ISNORMAL(3P)
